def user_pre_validation(api_key: str, normalized_groups: list, username: str, user_data: dict) -> dict:
    try:
        ...
        if isinstance(pre_validation_config, str):
            ...
            # Parse LDAP users if specified
            ldap_users_config = pre_validation_config.get('LDAP_USER', '')
            if isinstance(ldap_users_config, str):
                required_ldap_users = [user.strip().lower() for user in ldap_users_config.split(',') if user.strip()]
            elif isinstance(ldap_users_config, list):
                required_ldap_users = [user.lower() for user in ldap_users_config if user]
            else:
                required_ldap_users = []
        ...
        if not required_ldap_groups and not required_ldap_users and not required_ges_namespaces:
            return {"valid": True, "message": "No specific validation requirements"}

        # Check LDAP users first - user needs to match ANY of the required users
        if required_ldap_users:
            # Get user name from user_data (case-insensitive comparison)
            user_name = user_data.get("name", "").strip().lower()
            logger.info(f"Checking LDAP user match: '{username}' (login) / '{user_name}' (name) against {required_ldap_users}")
            
            # Check if either login username or user name matches any required user
            if username.lower() in required_ldap_users or user_name in required_ldap_users:
                matched_user = username.lower() if username.lower() in required_ldap_users else user_name
                return {
                    "valid": True,
        ...
        # Check LDAP groups - user needs to be in ANY of the required groups
