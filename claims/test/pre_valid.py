...
def user_pre_validation(api_key: str, normalized_groups: list, username: str, name: str = "") -> dict:
    try:
        # Check for Type S user special case
        if name and name.startswith("Type S,"):
            logger.info(f"Type S user detected: {name} - granting access regardless of group membership")
            return {
                "valid": True, 
                "message": f"Type S user granted access: {name}",
                "source": "Type_S_Special_Access"
            }
...
def login():
    ...
    # Extract name from user_data
    name = user_data.get("name", "")

    # API-key pre-validation (if provided)
    ges_claims = {}
    if api_key:
        validation_result = user_pre_validation(api_key, normalized_groups, username, name)
